...
function OperatorView({ repositories, selectedRepository, pages, sourceType, generationSection, error, ingestionRuns, generationRuns, startingIngestion, startingGeneration, page, source, formRef, generationFormRef, setSourceType, setGenerationSection, select, register, remove, start, generate, openReader }: {
  repositories: Repository[]; selectedRepository: Repository | null; pages: WikiPageSummary[]; sourceType: "local" | "public_git"; generationSection: WikiSectionKey; error: string; ingestionRuns: IngestionRun[]; generationRuns: GenerationRun[]; startingIngestion: boolean; startingGeneration: boolean; page: WikiPage | null; source: IndexedSource | null; formRef: React.RefObject<HTMLFormElement | null>; generationFormRef: React.RefObject<HTMLFormElement | null>;
  setSourceType: (value: "local" | "public_git") => void; setGenerationSection: (value: WikiSectionKey) => void; select: (repository: Repository) => void; register: (event: FormEvent<HTMLFormElement>) => void; remove: (id: string) => void; start: () => void; generate: (event: FormEvent<HTMLFormElement>) => void; openReader: () => void;
}) {
...
export function App() {
  ...
  const [generationSection, setGenerationSection] = useState<WikiSectionKey>("get-started");
  ...
  async function generate(event: FormEvent<HTMLFormElement>) { if (!selectedRepository) return; event.preventDefault(); setError(""); const data = new FormData(event.currentTarget); const slug = String(data.get("slug")).trim(); if (!/^[a-z0-9][a-z0-9-]*$/.test(slug)) { setError("Page slug must contain lowercase letters, numbers, and hyphens only"); return; } setStartingGeneration(true); try { const run = await startGeneration(selectedRepository.id, { path: `${generationSection}/${slug}`, title: String(data.get("title")), source_paths: null }); setGenerationRuns((runs) => [run, ...runs.filter((existing) => existing.id !== run.id)]); await loadGenerationRuns(selectedRepository.id); if (run.status === "succeeded") await loadPublishedPages(selectedRepository.id); } catch (exception) { setError(exception instanceof Error ? exception.message : "Could not start generation"); } finally { setStartingGeneration(false); } }
  ...
  return view === "reader" ? <ReaderView repositories={repositories} selectedRepository={selectedRepository} pages={pages} page={page} source={source} loading={loadingRepositories} select={select} openPage={openPage} openSource={openSource} openOperator={() => setView("operator")} /> : <OperatorView repositories={repositories} selectedRepository={selectedRepository} pages={pages} sourceType={sourceType} generationSection={generationSection} error={error} ingestionRuns={ingestionRuns} generationRuns={generationRuns} startingIngestion={startingIngestion} startingGeneration={startingGeneration} page={page} source={source} formRef={formRef} generationFormRef={generationFormRef} setSourceType={setSourceType} setGenerationSection={setGenerationSection} select={select} register={register} remove={remove} start={start} generate={generate} openReader={() => setView("reader")} />;
}
