...
type WikiSectionKey = typeof WIKI_SECTIONS[number]["key"];

export const FULL_WIKI_PAGES: ReadonlyArray<GenerationRequest> = [
  { path: "get-started/overview", title: "Get started", source_paths: null },
  { path: "concepts/architecture", title: "Architecture and concepts", source_paths: null },
  { path: "guides/configuration", title: "Configuration guide", source_paths: null },
  { path: "reference/api", title: "API reference", source_paths: null },
  { path: "workflows/publishing", title: "Publishing workflow", source_paths: null },
];

export function pageSection(path: string): WikiSectionKey | null {
  const [key, slug] = path.split("/", 2);
  return slug && WIKI_SECTIONS.some((section) => section.key === key) ? key as WikiSectionKey : null;
}

export function groupedPages(pages: WikiPageSummary[]) {
  return Object.fromEntries(WIKI_SECTIONS.map((section) => [section.key, pages.filter((page) => pageSection(page.path) === section.key)])) as Record<WikiSectionKey, WikiPageSummary[]>;
}
...
function WikiNavigation({ repository, pages, activePath, openPage }: { repository: Repository; pages: WikiPageSummary[]; activePath: string | undefined; openPage: (page: WikiPageSummary) => void }) {
  const [collapsed, setCollapsed] = useState<Set<WikiSectionKey>>(new Set());
  const groups = groupedPages(pages);
  return <nav className="page-navigation" aria-label="Wiki sections"><p className="eyebrow">In this wiki</p><h2>{repository.display_name}</h2>{WIKI_SECTIONS.map((section) => {
    const sectionPages = groups[section.key];
    const isCollapsed = collapsed.has(section.key);
    return <section className="wiki-section" key={section.key}><button className="section-toggle" aria-expanded={!isCollapsed} onClick={() => setCollapsed((current) => {
      const next = new Set(current);
      if (next.has(section.key)) next.delete(section.key); else next.add(section.key);
      return next;
    })}><span>{section.label}</span><small>{sectionPages.length} page{sectionPages.length === 1 ? "" : "s"}</small></button>{!isCollapsed && <div className="section-pages">{sectionPages.length === 0 ? <p className="empty-section">No published pages</p> : sectionPages.map((summary) => <button className={`page-link ${activePath === summary.path ? "active" : ""}`} key={summary.path} onClick={() => void openPage(summary)}>{summary.title}<span>{summary.path}</span></button>)}</div>}</section>;
  })}</nav>;
}
...
function ReaderView({ repositories, selectedRepository, pages, page, source, loading, select, openPage, openSource, openOperator }: {
  ...
  return <main className="reader-shell">
    ...
    {loading ? <section className="reader-empty" aria-live="polite"><p className="eyebrow">Reader</p><h2>Loading your documentation</h2><p>Fetching repositories and published pages.</p></section> : !selectedRepository ? <section className="reader-empty"><p className="eyebrow">Get started</p><h2>Choose a repository to read its wiki</h2><p>Register a repository, index its files, and generate a page from the operator dashboard.</p><button onClick={openOperator}>Open operator dashboard</button></section> : <section className="wiki-layout">
      <WikiNavigation repository={selectedRepository} pages={pages} activePath={page?.path} openPage={openPage} />
      <article className="reader-page">{page ? <><p className="eyebrow">{selectedRepository.display_name} / {page.path}</p><h2>{page.title}</h2><div className="reader-content">{renderDocument(page.content, page.diagrams)}</div>{page.citations.length > 0 && <footer className="citations"><div><p className="eyebrow">Traceability</p><h3>Sources</h3><p>Open a citation to inspect the indexed lines behind this page.</p></div><div className="citation-list">{page.citations.map((citation, index) => <button className="citation" key={`${citationLabel(citation)}-${index}`} onClick={() => void openSource(citation)}><span>{citation.path}</span><small>Lines {citation.line_start}–{citation.line_end}</small></button>)}</div></footer>}{source && <aside className="source-drawer" aria-label="Indexed source"><div><p className="eyebrow">Indexed source</p><h3>{source.path}</h3><p>{source.line_count} indexed lines</p></div><pre>{source.content}</pre></aside>}</> : <><h2>{pages.length === 0 ? "No published pages yet" : "Select a page"}</h2><p>{pages.length === 0 ? "Once ingestion is complete, generate a wiki page from the operator dashboard to see it here." : "Choose a published page from the navigation to start reading."}</p>{pages.length === 0 && <button onClick={openOperator}>Open operator dashboard</button>}</>}</article>
...
function OperatorView({ repositories, selectedRepository, pages, sourceType, error, ingestionRuns, generationRuns, startingIngestion, startingGeneration, page, source, formRef, setSourceType, select, register, remove, start, generate, openReader }: {
  repositories: Repository[]; selectedRepository: Repository | null; pages: WikiPageSummary[]; sourceType: "local" | "public_git"; error: string; ingestionRuns: IngestionRun[]; generationRuns: GenerationRun[]; startingIngestion: boolean; startingGeneration: boolean; page: WikiPage | null; source: IndexedSource | null; formRef: React.RefObject<HTMLFormElement | null>;
  setSourceType: (value: "local" | "public_git") => void; select: (repository: Repository) => void; register: (event: FormEvent<HTMLFormElement>) => void; remove: (id: string) => void; start: () => void; generate: () => void; openReader: () => void;
}) {
  ...
  const invalidPages = pages.filter((item) => pageSection(item.path) === null);
  return <main className="operator-shell"><header className="operator-header"><div><p className="eyebrow">HydraWiki operator</p><h1>Operator dashboard</h1><p>Manage repositories and inspect processing details.</p></div><button className="quiet-button" onClick={openReader}>Back to wiki reader</button></header>
...
export function App() {
  ...
  async function generate() { if (!selectedRepository) return; setError(""); setStartingGeneration(true); const failures: string[] = []; try { for (const payload of FULL_WIKI_PAGES) { try { const run = await startGeneration(selectedRepository.id, payload); setGenerationRuns((runs) => [run, ...runs.filter((existing) => existing.id !== run.id)]); if (run.status === "failed") failures.push(`${payload.path}: ${run.error ?? "generation failed"}`); } catch (exception) { failures.push(`${payload.path}: ${exception instanceof Error ? exception.message : "Could not start generation"}`); } } await Promise.all([loadGenerationRuns(selectedRepository.id), loadPublishedPages(selectedRepository.id)]); if (failures.length > 0) setError(failures.join("; ")); } finally { setStartingGeneration(false); } }
  ...
  return view === "reader" ? <ReaderView repositories={repositories} selectedRepository={selectedRepository} pages={pages} page={page} source={source} loading={loadingRepositories} select={select} openPage={openPage} openSource={openSource} openOperator={() => setView("operator")} /> : <OperatorView repositories={repositories} selectedRepository={selectedRepository} pages={pages} sourceType={sourceType} error={error} ingestionRuns={ingestionRuns} generationRuns={generationRuns} startingIngestion={startingIngestion} startingGeneration={startingGeneration} page={page} source={source} formRef={formRef} setSourceType={setSourceType} select={select} register={register} remove={remove} start={start} generate={generate} openReader={() => setView("reader")} />;
}
