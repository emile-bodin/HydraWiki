...
Mermaid fences are rendered by the local pinned Mermaid CLI before publication, with bounded input and a timeout; this uses no model or network service. The renderer process drops to the dedicated `hydrawiki-renderer` account and retains Chromium's sandbox; a missing account fails rendering closed. HydraWiki uses a fixed strict, non-HTML-label, default/classic/Dagre site configuration and locks the presentation-related Mermaid configuration keys. It accepts standard diagram structure and title-only frontmatter, but rejects per-diagram configuration, deprecated directives, and presentation syntax because CSS/classes cannot cross the inert-SVG boundary. The renderer SVG is untrusted until the backend accepts it against HydraWiki's minimal inert SVG vocabulary, which excludes CSS and `<style>`. Only a `safe` diagram with server-provided SVG is returned for display. A failed diagram stores its source and durable error on the generation run, and cannot replace an existing published page.
